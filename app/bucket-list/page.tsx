...
export default function BucketListPage() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Header & Progress Bar */}
      <div className="soft-card rounded-3xl p-6 sm:p-8 border border-border space-y-6">
        ...
        <div className="space-y-2 pt-2 border-t border-zinc-800">
          ...
          <div className="w-full h-3 rounded-full bg-zinc-900 overflow-hidden border border-zinc-800 p-0.5">
            <div
              className="h-full rounded-full transition-all duration-500 shadow-md shadow-rose-500/50"
      ...
      <div className="flex items-center space-x-2 border-b border-zinc-800 pb-3">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${ activeFilter === 'all' ? 'bg-rose-500/20 text-rose-300 border border-border' : 'text-zinc-400 hover:text-white' }`}
        >
          All Goals ({bucketList.length})
        </button>
        <button
          onClick={() => setActiveFilter('pending')}
          className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${ activeFilter === 'pending' ? 'bg-rose-500/20 text-rose-300 border border-border' : 'text-zinc-400 hover:text-white' }`}
        >
          Pending ({bucketList.length - completedCount})
        </button>
        <button
          onClick={() => setActiveFilter('completed')}
          className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${ activeFilter === 'completed' ? 'bg-rose-500/20 text-rose-300 border border-border' : 'text-zinc-400 hover:text-white' }`}
        >
      ...
      <div className="space-y-3">
        {filteredItems.map((item) => (
          <div
            key={item.id}
            className={`soft-card soft-card-hover rounded-2xl p-4 sm:p-5 border transition-all flex items-center justify-between gap-4 ${ item.completed ? 'border-border bg-emerald-500/5' : 'border-border' }`}
          >
            <div className="flex items-center space-x-4">
              <button
                onClick={() => handleToggle(item.id, item.completed)}
                className={`w-7 h-7 rounded-xl flex items-center justify-center border transition-all ${ item.completed ? 'bg-emerald-500 border-emerald-400 text-white shadow-lg shadow-emerald-500/30' : 'bg-zinc-900/80 border-zinc-700 hover:border-rose-500 text-transparent' }`}
              >
